...
public static class UISettingsProvider
{
    ...
    [MenuItem("GameFramework/Settings/UISettings", priority = -1)]
    public static void OpenSettings() => SettingsService.OpenProjectSettings("Project/GameFramework/UISettings");

    private const string SettingsPath = "Project/GameFramework/UISettings";
    ...
    private static void DrawHeader()
    {
        ...
        EditorGUILayout.LabelField(new GUIContent("UI代码生成器", "UI Script Generator Configuration"),
    ...
    private static void DrawCodeSettings(SerializedObject serializedObject)
    {
        ...
        if (m_showCodeSettings)
        {
            EditorGUILayout.BeginVertical("HelpBox");
            {
                ...
                var autoGenerateCodePathProperty = serializedObject.FindProperty("autoGenerateCodePath");
                autoGenerateCodePathProperty.stringValue = DrawEnhancedFolderField(
                    "自动生成代码保存路径",
                    "自动生成代码将生成到此目录",
                    autoGenerateCodePathProperty.stringValue);
                var groupCodePathProperty = serializedObject.FindProperty("groupCodePath");
                groupCodePathProperty.stringValue = DrawEnhancedFolderField(
                    "Group代码保存路径",
                    "Group代码将生成到此目录",
                    groupCodePathProperty.stringValue);
                ...
                var GroupNameProperty = serializedObject.FindProperty("_groupName");
                EditorGUILayout.PropertyField(GroupNameProperty,
                    new GUIContent("Group基类名", "UI组件的基类名称"));
            ...
            if (uiScriptGeneratorSettings != null)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    string genCodePath = string.IsNullOrEmpty(uiScriptGeneratorSettings.GenCodePath)
                        ? "未设置"
                        : uiScriptGeneratorSettings.GenCodePath;
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    string impCodePath = string.IsNullOrEmpty(uiScriptGeneratorSettings.ImpCodePath)
                        ? "未设置"
                        : uiScriptGeneratorSettings.ImpCodePath;
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.LabelField("自动生成脚本保存路径:", GUILayout.Width(80));
                    string impCodePath = string.IsNullOrEmpty(uiScriptGeneratorSettings.AutoGenerateCodePath)
                        ? "未设置"
                        : uiScriptGeneratorSettings.AutoGenerateCodePath;
                    EditorGUILayout.LabelField(impCodePath, EditorStyles.miniLabel);
                }
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.LabelField("组件脚本保存路径:", GUILayout.Width(80));
                    string groupCodePath = string.IsNullOrEmpty(uiScriptGeneratorSettings.GroupCodePath)
                        ? "未设置"
                        : uiScriptGeneratorSettings.GroupCodePath;
                    EditorGUILayout.LabelField(groupCodePath, EditorStyles.miniLabel);
                }
                EditorGUILayout.EndHorizontal();
            }
        }
        ...
        if (m_reorderableList == null)
        {
            m_reorderableList = new ReorderableList(serializedObject, ruleListProperty, true, true, true, true)
            {
                drawHeaderCallback = (Rect rect) =>
                {
                    ...
                    GUI.Label(new Rect(rect.x + columnWidth + padding + 20, rect.y, columnWidth, rect.height),
                        "UI元素名称前缀", EditorStyles.boldLabel);
                    GUI.Label(new Rect(rect.x + (columnWidth + padding) * 2 + 20, rect.y, columnWidth, rect.height),
                        "组件类型", EditorStyles.boldLabel);
                    GUI.Label(new Rect(rect.x + (columnWidth + padding) * 3, rect.y, columnWidth, rect.height),
                        "是否Group", EditorStyles.boldLabel);
                },
                ...
                drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                {
                    ...
                    // 组件类型
                    Rect componentRect = new Rect(rect.x + (columnWidth + padding) * 2, rect.y, columnWidth,
                        fieldHeight);
                    ...
                    // 是否Group
                    Rect GroupRect = new Rect(rect.x + (columnWidth + padding) * 3 + 10, rect.y, columnWidth,
                        fieldHeight);
                    SerializedProperty GroupProperty = element.FindPropertyRelative("isUIGroup");
                ...
                onAddCallback = (ReorderableList list) =>
                {
                    ...
                    newElement.FindPropertyRelative("isUIGroup").boolValue = false;
        ...
        if (m_uiGenTypeRecoedList == null)
        {
            m_uiGenTypeRecoedList = new ReorderableList(serializedObject, ruleListProperty, true, true, true, true)
            {
                drawHeaderCallback = (Rect rect) =>
                {
                    ...
                    GUI.Label(new Rect(rect.x + columnWidth + padding + 20, rect.y, columnWidth, rect.height),
                        "自动生成的UI类继承的类名", EditorStyles.boldLabel);
                },
                ...
                onAddCallback = (ReorderableList list) =>
                {
                    ...
                    // 设置默认值
                    SerializedProperty newElement = list.serializedProperty.GetArrayElementAtIndex(newIndex);
                    newElement.FindPropertyRelative("uiTypeName").stringValue = "UIView";
